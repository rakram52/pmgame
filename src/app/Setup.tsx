...
import { GREAT_OFFICES, INNER_MACHINE, SECOND_TIER_THEMES, DOCTRINE_DIALS, type OfficeChoice } from '../setup/content'
...
export function Setup({ onDone, onCancel }: { onDone: (g: GameState) => void; onCancel: () => void }) {
  ...
  function form() {
    const sel: SetupSelections = { pmName, offices, innerMachine, secondTierTheme: theme, doctrine, difficultyBias, modelProfile }
  ...
  return (
    <div class="setup screen-scroll">
      ...
      <p class="hint">Each pick locks a consequence that will fire somewhere in the first 10 weeks.</p>
      {DOCTRINE_DIALS.map((d) => (
        <div key={d.key} class="setup-office">
          <h3>{d.title}</h3>
          {d.note && <p class="stake">{d.note}</p>}
          <div class="cards">
            {d.options.map((o) => (
              <button key={o.key} class={`pick ${doctrine[d.key] === o.key ? 'sel' : ''}`} onClick={() => setDoctrine({ ...doctrine, [d.key]: o.key })}>
                <div class="pick-top">
